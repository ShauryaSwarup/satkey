use super::honk_verifier_circuits::{
    run_BN254_EVAL_FN_CHALLENGE_DUPL_53P_RLC_circuit,
    run_GRUMPKIN_HONK_PREP_MSM_SCALARS_SIZE_16_circuit,
    run_GRUMPKIN_HONK_SUMCHECK_SIZE_16_PUB_38_circuit,
};
...
mod UltraKeccakHonkVerifier {
    ...
    use garaga::utils::noir::{
        G2_POINT_KZG_1, G2_POINT_KZG_2, HonkProof, remove_unused_variables_sumcheck_evaluations,
    };
    use super::{
        precomputed_lines, run_BN254_EVAL_FN_CHALLENGE_DUPL_53P_RLC_circuit,
        run_GRUMPKIN_HONK_PREP_MSM_SCALARS_SIZE_16_circuit,
        run_GRUMPKIN_HONK_SUMCHECK_SIZE_16_PUB_38_circuit, vk,
    };
    ...
    impl IUltraKeccakHonkVerifier of super::IUltraKeccakHonkVerifier<ContractState> {
        fn verify_ultra_keccak_honk_proof(
            ...
            let (sum_check_rlc, honk_check) = run_GRUMPKIN_HONK_SUMCHECK_SIZE_16_PUB_38_circuit(
            ...
            let mut _points: Array<G1Point> = array![
                vk.qm, vk.qc, vk.ql, vk.qr, vk.qo, vk.q4, vk.qArith, vk.qDeltaRange, vk.qElliptic,
                vk.qAux, vk.qLookup, vk.qPoseidon2External, vk.qPoseidon2Internal, vk.s1, vk.s2,
                vk.s3, vk.s4, vk.id1, vk.id2, vk.id3, vk.id4, vk.t1, vk.t2, vk.t3, vk.t4,
                vk.lagrange_first, vk.lagrange_last, full_proof.proof.w1.into(),
                full_proof.proof.w2.into(), full_proof.proof.w3.into(), full_proof.proof.w4.into(),
                full_proof.proof.z_perm.into(), full_proof.proof.lookup_inverses.into(),
                full_proof.proof.lookup_read_counts.into(),
                full_proof.proof.lookup_read_tags.into(), full_proof.proof.z_perm.into(),
            ];
            ...
            let scalars: Span<u256> = array![
                scalar_1.try_into().unwrap(), scalar_2.try_into().unwrap(),
                scalar_3.try_into().unwrap(), scalar_4.try_into().unwrap(),
                scalar_5.try_into().unwrap(), scalar_6.try_into().unwrap(),
                scalar_7.try_into().unwrap(), scalar_8.try_into().unwrap(),
                scalar_9.try_into().unwrap(), scalar_10.try_into().unwrap(),
                scalar_11.try_into().unwrap(), scalar_12.try_into().unwrap(),
                scalar_13.try_into().unwrap(), scalar_14.try_into().unwrap(),
                scalar_15.try_into().unwrap(), scalar_16.try_into().unwrap(),
                scalar_17.try_into().unwrap(), scalar_18.try_into().unwrap(),
                scalar_19.try_into().unwrap(), scalar_20.try_into().unwrap(),
                scalar_21.try_into().unwrap(), scalar_22.try_into().unwrap(),
                scalar_23.try_into().unwrap(), scalar_24.try_into().unwrap(),
                scalar_25.try_into().unwrap(), scalar_26.try_into().unwrap(),
                scalar_27.try_into().unwrap(), scalar_28.try_into().unwrap(),
                scalar_29.try_into().unwrap(), scalar_30.try_into().unwrap(),
                scalar_31.try_into().unwrap(), scalar_32.try_into().unwrap(),
                scalar_33.try_into().unwrap(), scalar_34.try_into().unwrap(),
                scalar_35.try_into().unwrap(), scalar_44.try_into().unwrap(),
                scalar_45.try_into().unwrap(), scalar_46.try_into().unwrap(),
                scalar_47.try_into().unwrap(), scalar_48.try_into().unwrap(),
                scalar_49.try_into().unwrap(), scalar_50.try_into().unwrap(),
                scalar_51.try_into().unwrap(), scalar_52.try_into().unwrap(),
                scalar_53.try_into().unwrap(), scalar_54.try_into().unwrap(),
                scalar_55.try_into().unwrap(), scalar_56.try_into().unwrap(),
                scalar_57.try_into().unwrap(), scalar_58.try_into().unwrap(),
                scalar_59.try_into().unwrap(), scalar_72.try_into().unwrap(),
            ...
            let (zk_ecip_batched_lhs) = run_BN254_EVAL_FN_CHALLENGE_DUPL_53P_RLC_circuit(
                A0: random_point,
